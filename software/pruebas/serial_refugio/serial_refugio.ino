...
#define REFILL_TIME 3000  // rellenarBebedero: 5000 ms == 5s para rellenar bebedero (default)
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    String msg_rec = Serial.readString();
    msg_rec.trim();
    //Transceiver.GetStruct(&ebyte_msg, sizeof(ebyte_msg));

    // dump out what was just received
    Serial.println("Mensaje recibido por serial: " + msg_rec); //Serial.println(ebyte_msg.count);
...
void sendMessage(String data) {
  ...
  Serial.println("[Refugio] Enviado. ");
  //Serial.println("***");
  delay(1000);
}
