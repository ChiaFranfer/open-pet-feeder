...
%	\subsubsection{Simulación en \textit{Radio Mobile}}
	...
	\noindent El fabricante de la PCB es \href{https://jlcpcb.com/}{JLCPCB}[], y se usó su editor \href{https://easyeda.com/es}{EasyEDA}[], donde se implementaron los esquemáticos del prototipo. Una vez hecho, el programa lo traduce a una PCB, y tras unos retoques en enrutado, se pide al fabricante. Cuando llega, se comprueba su funcionamiento y se sueldan los componentes. Anotar que se han añadido conexiones extra a la PCB que no existían en el prototipo provisional, con el fin de facilitar las pruebas y/o futuras funcionalidades.\\
	
	\noindent Para mayor detalle del esquemático o de la PCB, consultar el repositorio de GitHub creado (poner  con href), donde se incluye un archivo .json que, importándolo a EasyEDA, permitirá su visualización. \\
	...
	\begin{figure}[h!]
		\begin{center}
			\includegraphics[width=0.3\textwidth]{img/prueba_lora_protoboard.png}
	...
	\noindent Se dejó el nodo transmisor transmitiendo mensajes con estos valores, mientras permanecía conectado a la corriente, en la segunda planta de un hogar (unos 5m, en el exterior). Por otro lado, el receptor iba conectado a una OLED donde mostraba los valores de \textit{Volts, Bits} y \textit{Count} del mensaje que recibía; la alimentación la recibía a través de una batería portátil.  \\
	...
	\begin{figure}[h!]
		\begin{center}
			\includegraphics[width=0.3\textwidth]{img/prueba_lora_protoboard_oled.png}
			\caption{}
	...
	\begin{figure}[h!]
		\begin{center}
			\includegraphics[width=0.3\textwidth]{img/prueba_1.jpg}
	...
	\noindent Para comprobar su funcionamiento, se usó el montaje mostrado en la Figura \ref{protoboard lora}. \\
	
	\subsubsubsection{Bebedero}
	
	\noindent \textbf{Bomba de agua}: se comprobó simplemente conectando su cable USB a la alimentación con un cargador, introduciéndola en un tarro con agua y un tubo transparente conectada a la salida de agua de la bomba, llevando el agua a un tarro vacío. Para uso posterior de la bomba de agua, se cortó el USB y se dejó los cables por separado de la bomba.\\
	
	\noindent \textbf{Relé}: \\
	
	\subsubsubsection{Comedero}
	
	\noindent \textbf{Servomotor}: \\
	
	\subsubsubsection{Sistema de alimentación autónoma}
	
	\noindent \textbf{Batería 18650}: \\
	
	\noindent \textbf{Panel solar y TP4056}: \\
	...
	\begin{itemize}
		...
		\item El límite inferior de potencia a la que transmite este transceptor es de 21dBm, pero el límite legal para emitir en la banda de LoRa, en Europa, es de 14dBm. Como ya se ha comentado anteriormente, habría que probar con otro transceptor muy similar, el E22-900T22D, ya que éste puede transmitir a menor potencia y,a demás, dar valores de RSSI, algo que no hemos podido medir en este proyecto por limitaciones del transceptor en este sentido.
...
%	\end{figure}
	...
	\addcontentsline{toc}{section}{Bibliografía}
	   ...
	   \subsection*{Enlaces y referencias}
		...
		\begin{enumerate}
			...
			\href{https://www.thingiverse.com/}{Thingiverse}
			...
			\item
			\label{easyeda web}
			\href{https://easyeda.com/es}{EasyEDA}
	...
	\noindent \textbf{Código Arduino usado para prueba de componentes} \\
	
	\noindent \textit{\textbf{Sensores de nivel de agua y relé}}: \textit{level\_switch\_test.ino} \\
	
	\begin{lstlisting}[language=C]
//Sensor de nivel tipo boya
//Salida a rele, para bomba de agua
...
void loop()
{
	...
	\end{lstlisting}
	\noindent \textit{\textbf{Bebedero}}: \textit{sketch\_waterPumpRelay.ino}\\
...
void loop() {
	...
	// para q se cojan datos de sensores y q se envie por lora
	...
	\noindent \textit{\textbf{Servomotor}}: \textit{sketch\_servoTest.ino}\\
...
void loop() {
	...
	\noindent \textit{\textbf{OLED}}: \textit{OLED\_display.ino}\\
...
void loop() {
	...
	\noindent \textit{\textbf{Comunicación LoRa}}: nodo de control, \textit{SendEBYTE.ino}\\
...
void loop() {
	...
	\noindent \textit{\textbf{Comunicación LoRa}}: nodo sensorización, \textit{ReceiveEBYTE\_OLED.ino}\\
