...
%	\subsubsection{Simulación en \textit{Radio Mobile}}
	...
	\noindent [Explicar página web, que hemos usado flask]. Para crear la página web, se han empleado como lenguajes de programación HTML y CSS; además, se ha hecho uso del framework conocido como \textit{Flask}, el cual permite un importante grado de personalización, idóneo para emprender proyectos web sencillos (aunque el framework ofrece la posibilidad de que escale al gusto de cada uno). Refs: [\ref{bib: flask serial}], [\ref{bib: flask doc}].
